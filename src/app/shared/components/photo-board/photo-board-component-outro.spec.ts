...
describe(PhotoBoardComponent.name + 'outros', () => {
  ...
  it(`Should display rows and columns when (@Input photos) has value`, () => {
    ...
    expect(component.board.rows[0].length).withContext('Number of columns from first row').toBe(4);
    expect(component.board.rows[1].length).withContext('Number of columns from second row').toBe(4);
  });
});
